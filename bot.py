...
def save_data():
    ...
    # DATA_FILE can be "leaderboard.json" or "/data/leaderboard.json".
    # If a folder path is used, create the folder automatically so the bot
    # does not crash when saving leaderboard data.
...
def is_valid_word(word: str) -> bool:
    # Fast mode: only local WORDLIST is checked.
    # The old online dictionary API waited up to 3 seconds, so replies felt slow.
    word = clean(word)
    if not word:
        return False
...
async def react_later(context: ContextTypes.DEFAULT_TYPE, chat_id: int, message_id: int):
    # Reaction ko background me bhejte hain, reply ko slow nahi karte.
    try:
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    asyncio.create_task(react_later(context, chat, msg.message_id))
