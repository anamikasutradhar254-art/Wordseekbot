...
# =========================
# KEYBOARD FIX (Play Again Button)
# =========================
...
"swing","bring","think","write","build","clean","throw","break","speak","learn","teach","dance","watch"
],
...
"switch","galaxy","cosmic","launch","create","delete","insert","update","render","process","thread","storage"
]
}

# =========================
# AUTO 300+ WORD EXPANDER
# =========================
def expand_wordlist(words, target=300, length=4):
    vowels = "aeiou"
...
WORDLIST = {
    4: expand_wordlist(BASE_WORDLIST[4], 300, 4),
    5: expand_wordlist(BASE_WORDLIST[5], 300, 5),
    6: expand_wordlist(BASE_WORDLIST[6], 300, 6),
}
...
async def new4(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🎮 4-letter game started!", reply_markup=GAME_KEYBOARD)

async def new5(update: Update, context: ContextTypes.DEFAULT_TYPE):
    new_game(update.effective_chat.id, 5)
    await update.message.reply_text("🎮 5-letter game started!", reply_markup=GAME_KEYBOARD)

async def new6(update: Update, context: ContextTypes.DEFAULT_TYPE):
    new_game(update.effective_chat.id, 6)
    await update.message.reply_text("🎮 6-letter game started!", reply_markup=GAME_KEYBOARD)
...
async def leaderboard_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not data:
        await update.message.reply_text("No scores yet!", reply_markup=GAME_KEYBOARD)
        return
    ...
    await update.message.reply_text(text, reply_markup=GAME_KEYBOARD)
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in game["guessed_words"]:
        await message.reply_text("❌ Word already used!", reply_markup=GAME_KEYBOARD)
        return
