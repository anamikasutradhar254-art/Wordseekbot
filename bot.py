...
# 🔥 STRICT VALIDATION (FIX FOR FAKE WORDS)
...
async def guess(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    expected_len = game["len"]

    # ❌ length check
    if len(text) != expected_len:
        await update.message.reply_text(
            f"❌ {raw} is not valid {expected_len}-letter word."
        )
        return
    ...
    if not text.isalpha():
        await update.message.reply_text(
            f"❌ {raw} is not a valid word!"
        )
        return

    # ❌ STRICT WORD BANK CHECK (MAIN FIX)
    if not is_valid_word(text, expected_len):
        await update.message.reply_text(
            f"❌ {raw} is not in word list!"
        )
        return
