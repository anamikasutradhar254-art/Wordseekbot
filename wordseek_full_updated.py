...
def save_data():
    ...
    save = {
        ...
        "group": {str(chat): dict(scores) for chat, scores in leaderboard.items()},
...
WORDSET = {4: set(WORDLIST[4]), 5: set(WORDLIST[5]), 6: set(WORDLIST[6])}
...
# Online dictionary check slow hota hai. Default OFF rakha hai taki user word bhejte hi bot instant reply kare.
# Agar kabhi API check chahiye ho to Railway/Render env me USE_ONLINE_DICTIONARY=true kar dena.
USE_ONLINE_DICTIONARY = os.getenv("USE_ONLINE_DICTIONARY", "false").lower() == "true"
...
def is_valid_word(word: str) -> bool:
    ...
    # Fast local check - yahi instant response dega.
    if word in WORDSET.get(len(word), set()):
        VALID_CACHE.add(word)
        return True

    # Slow internet API ko default OFF rakha hai.
    if not USE_ONLINE_DICTIONARY:
        INVALID_CACHE.add(word)
        return False

    try:
        # 3 sec timeout se bot slow ho raha tha, isliye short timeout.
        res = requests.get(f"https://api.dictionaryapi.dev/api/v2/entries/en/{word}", timeout=0.8)
...
async def add_fast_reaction(context: ContextTypes.DEFAULT_TYPE, chat_id: int, message_id: int):
    try:
...
def save_user(update: Update):
    ...
    if not user:
        return
    uid = user.id
    user_names[uid] = user.username or user.first_name or f"User{uid}"


def is_banned(update: Update):
    user = update.effective_user
    return bool(user and user.id in banned_users)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(caption, reply_markup=start_keyboard(context.bot.username))
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.edit_text(text, parse_mode="HTML", reply_markup=leaderboard_keyboard())
...
async def start_game_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE, length: int):
    if is_banned(update):
        await update.message.reply_text("🚫 You are banned from using this bot.")
        return
    ...
    new_game(update.effective_chat.id, length)
    await update.message.reply_text(
...
async def add_points(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        points = int(context.args[0]) if update.message.reply_to_message else int(context.args[1])
    except Exception:
...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.reply_to_message:
        ...
        for chat_id in all_chats:
            try:
                if source.photo:
                    await context.bot.send_photo(chat_id=chat_id, photo=source.photo[-1].file_id, caption=caption)
                elif source.video:
                    await context.bot.send_video(chat_id=chat_id, video=source.video.file_id, caption=caption)
                elif source.animation:
                    await context.bot.send_animation(chat_id=chat_id, animation=source.animation.file_id, caption=caption)
                elif source.document:
                    await context.bot.send_document(chat_id=chat_id, document=source.document.file_id, caption=caption)
                elif source.text:
                    await context.bot.send_message(chat_id=chat_id, text=source.text)
                else:
                    failed += 1
                    continue
                sent += 1
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Sirf pure English word count hoga.
    # Examples:
    #   Coil    -> count hoga
    #   Coil 21 -> ignore
    #   Coil..  -> ignore
    ...
    # Reaction ko await nahi karte, warna reply slow feel hota hai.
    context.application.create_task(add_fast_reaction(context, chat, msg.message_id))
    ...
    if text == game["word"]:
        ...
        await msg.reply_text(win_text, reply_to_message_id=msg.message_id)
        return
    ...
    if game["tries"] >= 30:
        ...
        await msg.reply_text(lose_text, reply_to_message_id=msg.message_id)
        return

    await msg.reply_text(build_history(game), reply_to_message_id=msg.message_id)
